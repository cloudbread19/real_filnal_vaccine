...
class Engine:
    ...
    def __init__(self, debug=False):
        ...
        # 플러그 엔진의 가장 최신 시간 값을 가짐
        # 초기값으로는 1980-01-01을 지정한다. 임의로 지정해놓음
    ...
    def set_plugins(self, plugins_path):
        ...
        if not priority: # 로딩할 clb 파일이 없을 시
            return False
    ...
    # 복호화 된 플러그인 엔진의 빌드 시간 값 중 가장 최신 값을 보관함
...
class EngineInstance:
    ...
    def make_instance(self, modules):  # 백신 엔진 인스턴스를 생성
        for mod in modules:
            try:
                t = mod.CLBMain()  # 각 플러그인 CLBMain 인스턴스 생성
                self.clbmain_instance.append(t)
            except AttributeError:  # CLBMain 클래스 존재하지 않음
                continue

        if len(self.clbmain_instance):  # CLBMain 인스턴스가 하나라도 있으면 성공
            if self.debug:
    ...
    # 입력값 : filename - 악성코 검사 대상 파일 또는 폴더 이름,   callback - 검사 시 출력 화면 관련 콜백 함수
    # 리턴값 : 0 - 성공
